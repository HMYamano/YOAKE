"""
visualize_stage1_val.py — Stage 1 モデルで validation データを解析・可視化

モデル予測 (緑) と GT (赤) を重ねて画像に描画して保存する。
メトリクス (AP50, AP75, recall など) も計算して出力する。
...
def draw_boxes_on_image(
    ...
    pred_boxes_xyxy: torch.Tensor,   # (N_pred, 4) 正規化 xyxy [0,1]
    pred_scores: torch.Tensor,       # (N_pred,)
    gt_boxes_xyxy: torch.Tensor,     # (N_gt, 4) pixel coords xyxy
    image_size: tuple[int, int],     # (H, W)
) -> Image.Image:
    """テンソルから PIL 画像を作り、GT (赤) と予測 (緑) BBox を描画して返す。"""
    ...
    for i, box in enumerate(pred_boxes_xyxy):
        ...
        draw.rectangle([text_pos[0], text_pos[1], text_pos[0] + 36, text_pos[1] + 13],
                       fill=COLOR_PRED)
        draw.text(text_pos, label, fill=(0, 0, 0), font=ImageFont.load_default())
...
def main():
    ...
    print(f"Analyzing Stage 1 on VAL data")
    ...
    total_objects = sum(len(f.objects) for v in videos for f in v.frames)
    print(f"\nDataset info:")
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        for b in range(B):
            ...
            if images_saved < max_images:
                ...
                draw.text((5, 5),  f"GT({n_gt})   red",    fill=COLOR_GT,   font=_font)
                draw.text((5, 18), f"Pred({n_pred}) green", fill=COLOR_PRED, font=_font)
    ...
    print("\n=== Stage 1 Detection Metrics (VAL data) ===")
    ...
    summary_path = output_dir / "summary_val.json"
